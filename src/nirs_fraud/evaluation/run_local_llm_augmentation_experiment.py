...
DEFAULT_ARCHITECTURES = ("llm_checklist", "llm_self_check", "llm_ensemble")
...
def write_summary(path: Path, metrics_rows: list[dict]) -> None:
    lines = [
        ...
        "",
        "| Model | Architecture | Variant | Accuracy | Recall fraud | FP | FN |",
        "|---|---|---|---:|---:|---:|---:|",
    ]
    for row in metrics_rows:
        lines.append(
            f"| {row['model']} | {row['architecture']} | {row['variant']} | {row['accuracy']:.3f} | {row['recall_fraud']:.3f} | {row['false_positives']} | {row['false_negatives']} |"
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
