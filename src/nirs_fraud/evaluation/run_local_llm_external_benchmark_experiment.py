...
def write_per_architecture_outputs(output_dir: Path, model_slug: str, predictions: list[dict], metrics_rows: list[dict]) -> None:
    ...
    for architecture in architectures:
        ...
        write_csv(
            output_dir / f"local_llm_experiment_03_{model_slug}_{architecture}_predictions.csv",
            architecture_predictions,
        )
        write_csv(
            output_dir / f"local_llm_experiment_03_{model_slug}_{architecture}_metrics.csv",
            architecture_metrics,
        )
        write_summary(
            output_dir / f"local_llm_experiment_03_{model_slug}_{architecture}_summary.md",
            architecture_metrics,
        )
