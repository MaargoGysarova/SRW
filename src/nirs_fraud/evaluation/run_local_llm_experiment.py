...
def run(
    *,
    ...
    for architecture in architectures:
        ...
        total_rows = len(rows)
        render_progress(f"exp1:{architecture}", completed=0, total=total_rows)
        for row_index, row in enumerate(rows, start=1):
            ...
            render_progress(f"exp1:{architecture}", completed=row_index, total=total_rows)
        finish_progress()
        metrics_rows.append(
            {
                "experiment": "experiment_01_internal_synthetic_core",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run experiment 1 with a local GPU LLM via transformers.")
